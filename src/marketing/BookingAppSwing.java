...
public class BookingAppSwing extends JFrame {
    ...
    private JPanel createSingleBookingPanel() {
        ...
        JComboBox<String> roomBox = new JComboBox<>(
                new String[] { "Main Hall - Stalls", "Main Hall - Balcony", "Small Hall", "Rehearsal Room" });
        ...
        Runnable updateSeatGrid = () -> {
            ...
            List<String> allSeats = room.contains("Main Hall") ? generateMainHallSeats()
                    : room.contains("Small Hall") ? generateSmallHallSeats() : new ArrayList<>();
        ...
        confirmBtn.addActionListener((ActionEvent e) -> {
            try {
                ...
                String timeStr = (String) timeBox.getSelectedItem();
                if (date == null || timeStr == null) {
                    status.setText("⚠️ Select date and time.");
                    return;
                }
                LocalDateTime bookingTime = LocalDateTime.of(date, LocalTime.parse(timeStr));
                ...
                if (room == null || room.isEmpty()) {
                    status.setText("⚠️ Select a room.");
                    return;
                }
                ...
                if (selectedSeat == null) {
                    status.setText("❌ Please select a seat.");
                    return;
                }

                if (selectedSeat.matches("R.*")) {
                    status.setText("❌ Restricted view seat.");
                    return;
                }

                if (wheelchair && !(selectedSeat.startsWith("A") || selectedSeat.startsWith("L")
                        || selectedSeat.endsWith("1") || selectedSeat.endsWith("10"))) {
                    status.setText("⚠️ Accessible seats only on Row A/L or edges.");
                    return;
                }
                ...
                System.out.println(
                        "Attempting to confirm: " + name + ", " + bookingTime + ", " + selectedSeat + ", " + price);
                ...
                if (isSeatAvailable && singleService.confirmSingleBooking(booking)) {
                    ...
                    // MySQLDatabaseHelper.saveSingleBooking(booking.getCustomerName(),
                    // Timestamp.valueOf(bookingTime), booking.getSeatNumber(), booking.getPrice());
    ...
    private JPanel createGroupBookingPanel() {
        ...
        final JComboBox<String> sessionBox = new JComboBox<>(
                new String[] { "1 Hour", "Morning", "Afternoon", "All Day" });
        ...
        confirmBtn.addActionListener((ActionEvent e) -> {
            ...
            if (groupService.confirmGroupBooking(fullName)) {
                groupService.getAllBookings().stream()
                        .filter(b -> b.getGroupName().equals(fullName) && "Confirmed".equals(b.getStatus())).findFirst()
                        .ifPresent(booking -> {
        ...
        form.add(new JLabel("Group Name:"));
        form.add(groupName);
        form.add(new JLabel("Group Size:"));
        form.add(groupSize);
        form.add(new JLabel("Date:"));
        form.add(dateSpinner);
        form.add(new JLabel("Time:"));
        ...
        panel.add(sessionBox); // ✅ <-- New line

        panel.add(sessionLabel);
        panel.add(sessionBox); // ✅ Add herepanel.add(sessionBox);
        panel.add(sessionLabel);
        form.add(priceLabel);
        form.add(groupStatus);
        form.add(holdBtn);
        form.add(confirmBtn);
        ...
        form.add(refreshBtn);
        form.add(new JLabel()); // To fill grid slot
    ...
    private boolean isSeatBooked(String seat, LocalDateTime time) {
        ...
        return confirmedSingleBookings.stream().anyMatch(b -> b.getSeatNumber().contains(seat) &&
                !b.getBookingTime().isBefore(start) &&
                !b.getBookingTime().isAfter(end))
                || confirmedGroupBookings.stream().anyMatch(b -> b.getHeldRows().contains(seat) &&
                        !b.getBookingTime().isBefore(start) &&
                        !b.getBookingTime().isAfter(end));
    }
    ...
    public JPanel getBookingPanel() {
        ...
        clockLabel.setHorizontalAlignment(SwingConstants.RIGHT);
        clockLabel.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 10));
        panel.add(clockLabel, BorderLayout.SOUTH);
